...
class Repository:
    ...
    def __init__(self, dir_path, pt=True):
        ...
        if pt:
            ...
            print("\nInstructor Summary (via database)")
            self.inst_ptable()
    ...
    def instructor_table_db(self):
        """ read instructor data from created database """

        pt = PrettyTable(field_names=Instructor.pt_hdr)

        db_path = "/Users/jigar/sqlite/startup.db"

        db = sqlite3.connect(db_path)

        for rows in db.execute("select instructors.CWID, instructors.Name, instructors.Dept, grades.Course, count(*) as Number_of_Student from instructors join grades on instructors.CWID = grades.InstructorCWID group by Course, InstructorCWID"):
            pt.add_row(rows)
    ...
    def inst_ptable(self):
        """ print new instructor pretty table """
...
def main():
    ...
    stevens = r"/Users/jigar/Documents/SSW 810/Project/PH11"
